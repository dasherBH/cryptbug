...
if ( ! function_exists( 'engitech_woocommerce_cart_link_fragment' ) ) {
	...
	function engitech_woocommerce_cart_link_fragment( $fragments ) {
		global $woocommerce;
		ob_start();
		engitech_woocommerce_cart_link();
...
if ( ! function_exists( 'engitech_woocommerce_cart_link' ) ) {
	/**
	 * Cart Link.
	 *
	 ...
	 * @return void
	 */
	function engitech_woocommerce_cart_link() {
		?>
			<a class="cart-icon" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'engitech' ); ?>">
				<i class="flaticon-shopper"></i>
				<?php
					$item_count_text = sprintf(
						/* translators: number of items in the mini cart. */
						_n( '%d', '%d', WC()->cart->get_cart_contents_count(), 'engitech' ),
